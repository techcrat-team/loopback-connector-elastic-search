...
ESConnector.prototype.connect = function (callback) {
  ...
  if (self.db) {
    ...
    if (self.settings.mappings) {
      /**
      * Custom Settings: Dont migrate indexes on connect. Indexes will be created on running migration scripts. Check autoupdate.
      ****/
      // self.setupMappings()
      //   .then(function () {
...
ESConnector.prototype.addDefaults = function (modelName, functionName) {
  ...
  //Adding custom logic to pickup indexname, type name and settings from model.json file

  if(modelClass && _.isObject(modelClass.settings.elasticsearch)) {
    filter.index = modelClass.settings.elasticsearch.index || '';
    filter.type = modelClass.settings.elasticsearch.type || '';
  }

  // if (modelClass && _.isObject(modelClass.settings.elasticsearch) && _.isObject(modelClass.settings.elasticsearch.settings)) {
  //   _.extend(filter, { settings : modelClass.settings.elasticsearch.settings });
  // }
...
/**
 * Perform autoupdate for the given models. Create/recreate DB objects
 * (such as table/column/constraint/trigger/index) to match the model definitions
 *  --> Drop the corresponding indices: both mappings and data are done away with
 *  --> create/recreate mappings and indices
 *
 ...
 * @param {Function} [cb] The callback function
 */
ESConnector.prototype.autoupdate = require('./autoupdate.js')({
  log: log
  , lodash: _
  , bluebird: Promise
});
